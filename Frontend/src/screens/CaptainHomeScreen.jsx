...
function CaptainHomeScreen() {
  ...
  const acceptRide = async () => {
    try {
      if (newRide._id != "") {
        ...
        setShowBtn("otp");
        setMapLocation({
          center: [riderLocation.ltd, riderLocation.lng],
          pickup: newRide.pickupCoords ? [newRide.pickupCoords.ltd, newRide.pickupCoords.lng] : null,
          destination: null,
          captain: [riderLocation.ltd, riderLocation.lng]
        });
    ...
    try {
      if (newRide._id != "" && otp.length == 6) {
        ...
        setMapLocation({
          center: [riderLocation.ltd, riderLocation.lng],
          pickup: null,
          destination: newRide.destinationCoords ? [newRide.destinationCoords.ltd, newRide.destinationCoords.lng] : null,
          captain: [riderLocation.ltd, riderLocation.lng]
        });
        setShowBtn("end-ride");
        setLoading(false);
      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          setMapLocation({
            ...
            pickup: null,
            destination: null,
            captain: [position.coords.latitude, position.coords.longitude]
          });
  ...
  return (
    ...
    >
      ...
      />
      <Sidebar />
      <div className="map w-full h-[80vh] z-0">
        <LiveMap 
          center={mapLocation.center}
          pickup={mapLocation.pickup}
          destination={mapLocation.destination}
          captain={mapLocation.captain}
        />
      </div>
