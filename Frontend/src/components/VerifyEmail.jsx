...
function VerifyEmail({ user, role }) {
    ...
    const sendVerificationEmail = async () => {
        try {
            ...
            if (response.status === 200 && response.data.mailData) {
                // Now send the actual email using our Vercel Serverless Function via Resend
                const sendRes = await axios.post('/api/sendEmail', response.data.mailData);
            ...
            showAlert('Some error occured', errorMessage, 'failure');
